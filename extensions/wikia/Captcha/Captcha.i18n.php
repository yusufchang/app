...
# * Toute ligne non vide est un fragment d'expression rationnelle qui ne correspondra qu'aux hôtes dans les liens hypertextes
 #</pre><!-- laissez cette ligne exactement telle quelle -->",
	...
	'recaptcha-misconfigured' => "ReCaptcha n'est pas configuré correctement",
);
...
$messages['ja'] = array(
	...
	'captcha-desc' => 'スパムやパスワードの推測から保護するためのCAPTCHA（画像認証）技術を提供します',
	'captcha-addurl' => '編集内容に新しい外部リンクが含まれています。
自動化されたスパムから保護するため、下の簡単な計算を解いて答えをボックスに入力してください
（[[Special:Captcha/help|詳細情報]]）:',
	'captcha-badlogin' => '自動化されたパスワードの解読から保護するため、下の簡単な計算を解いて答えをボックスに入力してください
（[[Special:Captcha/help|詳細情報]]）:',
	'captcha-createaccount' => '自動化されたアカウント作成から保護するため、下の簡単な計算を解いて答えをボックスに入力してください
（[[Special:Captcha/help|詳細情報]]）:',
	...
	'captcha-sendemail' => '自動化されたスパムから保護するため、下の簡単な計算を解いて答えをボックスに入力してください（[[Special:Captcha/help|詳細情報]]）:',
...
このようなスパムのリンクは削除可能ですが、非常に迷惑なことです。

場合によっては（特に新しいウェブリンクをページに追加する際など）、色付きの文字列や歪んだ文字列の画像が表示され、その文字列の入力を求められることがあります。
この操作は自動化することが難しいため、ほとんどの本物の人間なら投稿を行える一方で、スパムやロボットの攻撃を阻止することができます。

視覚に障害のある方やテキストベースまたは音声ベースのブラウザをお使いの方にはご不便をおかけいたしますが、現時点では、音声による代替方法はご用意していません。
画像認証のために正当な投稿を行えずサポートが必要な場合は、[[{{MediaWiki:Grouppage-sysop}}|サイト管理者]] にご連絡ください。

ブラウザの戻るボタンをクリックすると、ページの編集画面に戻ります。',
	'captcha-addurl-whitelist' => ' #<!-- この行は変更不可 --> <pre>
# 構文は次のとおりです:
#   * 「#」の文字から行の末尾までのすべてがコメントです
#   * 空白以外の行はすべて、URL内のホストのみとマッチする正規表現のフラグメントです
 #</pre> <!-- この行は変更不可 -->',
	'right-skipcaptcha' => 'CAPTCHAを行わずにCAPTCHAのトリガ操作を実行します',
	'captcha-input-placeholder' => 'ここに文字列を入力してください',
	'captcha-no-image' => '画像認証用の画像が見つかりませんでした',
	'recaptcha-desc' => '編集の確認用reCAPTCHAモジュール',
	'recaptcha-edit' => '自動化された編集スパムから保護するため、下の画像認証を行ってください:',
	'recaptcha-addurl' => '編集内容に新しい外部リンクが含まれています。自動化されたスパムから保護するため、下の画像認証を行ってください:',
	'recaptcha-badpass' => '自動化されたパスワードの解読から保護するため、下の画像認証を行ってください:',
	'recaptcha-createaccount' => '自動化されたアカウント作成から保護するため、下の画像認証を行ってください:',
	'recaptcha-createaccount-fail' => 'reCAPTCHAの回答が間違っているか入力されていません。',
	'recaptcha-create' => '自動化されたページ作成から保護するため、下の画像認証を行ってください:',
	'recaptcha-misconfigured' => 'reCaptchaが正しく構成されていません',
);
...
#   * 모든 비어 있지 않은 줄은 URL과 일치하는 정규 표현식 단편(Regex Fragment)입니다.
 #</pre> <!-- 이 줄은 수정하지 마세요 -->',
	...
	'recaptcha-misconfigured' => '리캡차가 정상적으로 설정되어 있지 않습니다',
);
...
',
	...
	'recaptcha-misconfigured' => 'ReCaptcha nie jest poprawnie skonfigurowana',
);
...
#   * Каждая непустая строка является фрагментом regex, который будет совпадать с хостами внутри URL
 #</pre> <!--не изменяйте этот текст -->',
	...
	'recaptcha-misconfigured' => 'Неправильная настройка ReCaptcha',
);
...
# * 所有非空白行是一个正则表达式，只会对URL中的内容进行匹配
 #</pre> <!-- 不要更改本行的內容 -->',
	...
	'recaptcha-misconfigured' => '验证码未正确放置',
);
