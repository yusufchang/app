    									   ...
    									   $mustMatch, $mustNotMatch, $matchParsed, $iTitleMaxLen, $catlist)
    {
        ...
        if (count($tCalls) <= 1) {
            ...
            if (count($extractParm)>0) {
                ...
                else $output[0]=$dpl->formatTemplateArg('',$dplNr,0,true,-1,$article);
            } else {
                ...
                for ($i=0; $i<$size;$i++) {
                    ...
                    if ($cbrackets==0) {
                        ...
                        if (($mustMatch   =='' ||  preg_match($mustMatch,substr($templateCall,0,$i-1))) && 
							...
							if ($catlist!='') $argChain .= "|%CATLIST%=$catlist";
                    ...
                    if ($cbrackets==0) {
                        ...
                        if ( ($mustMatch   =='' || 
                            ...
                            foreach ($extractParm as $exParmKey => $exParm) {
                                ...
                                if (!$found) $output[$n] .= $dpl->formatTemplateArg('',$dplNr,$exParmKey,$firstCall,$maxlen,$article);
                                $second=true;
                            }
                        }
                        break;
