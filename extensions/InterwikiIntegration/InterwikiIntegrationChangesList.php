 ...
 * @todo document
 */
class InterwikiIntegrationRCCacheEntry extends InterwikiIntegrationRecentChange {
	var $secureName, $link;
	var $curlink , $difflink, $lastlink, $usertalklink, $versionlink;
	var $userlink, $timestamp, $watched;

	static function newFromParent( $rc ) {
		$rc2 = new InterwikiIntegrationRCCacheEntry;
 ...
 * - recent changes
 */
class InterwikiIntegrationChangesList {
	...
	*/
	 ...
	 * @return InterwikiIntegrationChangesList derivative
	 */
	public static function newFromUser( &$user ) {
		...
		$sk = $user->getSkin();
		$list = null;
		if( wfRunHooks( 'FetchInterwikiIntegrationChangesList', array( &$user, &$sk, &$list ) ) ) {
			$new = $wgRequest->getBool( 'enhanced', $user->getOption( 'usenewrc' ) );
			return $new ? new EnhancedInterwikiIntegrationChangesList( $sk ) : new OldInterwikiIntegrationChangesList( $sk );
		} else {
			return $list;
		}
	}
	 ...
	 * @return String
	 */
	public function beginRecentInterwikiIntegrationChangesList() {
		$this->integration_rc_cache = array();
		$this->rcMoveIndex = 0;
		$this->rcCacheIndex = 0;
		$this->lastdate = '';
	 ...
	 * @return String
	 */
	public function endRecentInterwikiIntegrationChangesList() {
		if( $this->rclistOpen ) {
			return "</ul>\n";
		} else {
			return '';
	...
	public function insertMove( &$s, $rc ) {
		...
		# "[[x]] moved to [[y]]"
		$msg = ( $rc->mAttribs['integration_rc_type'] == RC_MOVE ) ? '1movedto2' : '1movedto2_redir';
		$s .= wfMsg(
			$msg,
			$this->skin->link(
				$rc->getTitle(),
	...
	public function insertDateHeader( &$s, $integration_rc_timestamp ) {
		global $wgLang;
		# Make date header if necessary
		$date = $wgLang->date( $integration_rc_timestamp, true, true );
	...
	public function insertDiffHist( &$s, &$rc, $unpatrolled ) {
		...
		if( $rc->mAttribs['integration_rc_type'] == RC_NEW || $rc->mAttribs['integration_rc_type'] == RC_LOG ) {
			...
			$query = array(
				'curid' => $rc->mAttribs['integration_rc_cur_id'],
				'diff'  => $rc->mAttribs['integration_rc_this_oldid'],
				'oldid' => $rc->mAttribs['integration_rc_last_oldid']
			);

			if( $unpatrolled ) {
				$query['rcid'] = $rc->mAttribs['integration_rc_id'];
			};
	...
	public function insertArticleLink( &$s, &$rc, $unpatrolled, $watched ) {
		...
		if ( $unpatrolled && $rc->mAttribs['integration_rc_type'] == RC_NEW ) {
			$params['rcid'] = $rc->mAttribs['integration_rc_id'];
		}
		...
		wfRunHooks( 'InterwikiIntegrationChangesListInsertArticleLink',
	...
	public function insertUserRelatedLinks( &$s, &$rc ) {
		if( $this->isDeleted( $rc, Revision::DELETED_USER ) ) {
		  ...
		  $s .= $this->skin->userLink( $rc->mAttribs['integration_rc_user'], $rc->mAttribs['integration_rc_user_text'] );
		  $s .= $this->skin->userToolLinks( $rc->mAttribs['integration_rc_user'], $rc->mAttribs['integration_rc_user_text'] );
		}
	}
	...
	public function insertAction( &$s, &$rc ) {
		if( $rc->mAttribs['integration_rc_type'] == RC_LOG ) {
			if( $this->isDeleted( $rc, LogPage::DELETED_ACTION ) ) {
				...
				$s .= ' '.LogPage::actionText( $rc->mAttribs['integration_rc_log_type'], $rc->mAttribs['integration_rc_log_action'],
					$rc->getTitle(), $this->skin, LogPage::extractParams( $rc->mAttribs['integration_rc_params'] ), true, true );
			}
		}
	}
	...
	public function insertComment( &$s, &$rc ) {
		if( $rc->mAttribs['integration_rc_type'] != RC_MOVE && $rc->mAttribs['integration_rc_type'] != RC_MOVE_OVER_REDIRECT ) {
			if( $this->isDeleted( $rc, Revision::DELETED_COMMENT ) ) {
				$s .= ' <span class="history-deleted">' . wfMsgHtml( 'rev-deleted-comment' ) . '</span>';
			} else {
				$s .= $this->skin->commentBlock( $rc->mAttribs['integration_rc_comment'], $rc->getTitle() );
			}
		}
	}
	...
	public static function isDeleted( $rc, $field ) {
		return ( $rc->mAttribs['integration_rc_deleted'] & $field ) == $field;
	}
	...
	public function insertRollback( &$s, &$rc ) {
		global $wgUser;
		if( !$rc->mAttribs['integration_rc_new'] && $rc->mAttribs['integration_rc_this_oldid'] && $rc->mAttribs['integration_rc_cur_id'] ) {
			$page = $rc->getTitle();
			/** Check for rollback and edit permissions, disallow special pages, and only
			  * show a link on the top-most revision */
			if ($wgUser->isAllowed('rollback') && $rc->mAttribs['integration_page_latest'] == $rc->mAttribs['integration_rc_this_oldid'] )
			{
				$rev = new Revision( array(
					'id'        => $rc->mAttribs['integration_rc_this_oldid'],
					'user'      => $rc->mAttribs['integration_rc_user'],
					'user_text' => $rc->mAttribs['integration_rc_user_text'],
					'deleted'   => $rc->mAttribs['integration_rc_deleted']
				) );
				$rev->setTitle( $page );
 ...
 */
class OldInterwikiIntegrationChangesList extends InterwikiIntegrationChangesList {
	/**
	...
	public function recentChangesLine( &$rc, $watched = false, $linenumber = null ) {
		...
		$unpatrolled = $wgUser->useRCPatrol() && !$rc->mAttribs['integration_rc_patrolled'];

		$dateheader = ''; // $s now contains only <li>...</li>, for hooks' convenience.
		$this->insertDateHeader( $dateheader, $rc->mAttribs['integration_rc_timestamp'] );
		...
		// Log entries
		} elseif( $rc->mAttribs['integration_rc_log_type'] ) {
			$logtitle = Title::newFromText( 'Log/'.$rc->mAttribs['integration_rc_log_type'], NS_SPECIAL );
			$this->insertLog( $s, $logtitle, $rc->mAttribs['integration_rc_log_type'] );
		// Log entries (old format) or log targets, and special pages
		} elseif( $rc->mAttribs['integration_rc_namespace'] == NS_SPECIAL ) {
			list( $name, $subpage ) = SpecialPageFactory::resolveAlias( $rc->mAttribs['integration_rc_title'] );
			if( $name == 'Log' ) {
		...
		// Regular entries
			...
			$s .= $this->recentChangesFlags( $rc->mAttribs['integration_rc_new'], $rc->mAttribs['integration_rc_minor'],
				$unpatrolled, '', $rc->mAttribs['integration_rc_bot'] );
 ...
 */
class EnhancedInterwikiIntegrationChangesList extends InterwikiIntegrationChangesList {
	/**
	...
	public function recentChangesLine( &$baseRC, $watched = false ) {
		...
		# Extract fields from DB into the function scope (integration_rc_xxxx variables)
		// FIXME: Would be good to replace this extract() call with something
		// that explicitly initializes variables.
		extract( $rc->mAttribs );
		$curIdEq = array( 'curid' => $integration_rc_cur_id );

		# If it's a new day, add the headline and flush the cache
		$date = $wgLang->date( $integration_rc_timestamp, true );
		$ret = '';
		if( $date != $this->lastdate ) {
		...
		if( $wgUser->useRCPatrol() ) {
		  	$rc->unpatrolled = !$integration_rc_patrolled;
		} else {
		...
		# Make article link
		// Page moves
		if( $integration_rc_type == RC_MOVE || $integration_rc_type == RC_MOVE_OVER_REDIRECT ) {
			$msg = ( $integration_rc_type == RC_MOVE ) ? "1movedto2" : "1movedto2_redir";
		...
		} elseif( $integration_rc_namespace == NS_SPECIAL ) {
			list( $specialName, $logtype ) = SpecialPageFactory::resolveAlias( $integration_rc_title );
			if ( $specialName == 'Log' ) {
				# Log updates, etc
				$logname = LogPage::logName( $logtype );
		...
		if ( !InterwikiIntegrationChangesList::userCan($rc,Revision::DELETED_TEXT) ) {
			$showdifflinks = false;
		}

		$time = $wgLang->time( $integration_rc_timestamp, true, true );
		$rc->watched = $watched;
		$rc->link = $clink;
		$rc->timestamp = $time;
		...
		if( $rc->unpatrolled ) {
			$rcIdQuery = array( 'rcid' => $integration_rc_id );
		} else {
			$rcIdQuery = array();
		}
		$querycur = $curIdEq + array( 'diff' => '0', 'oldid' => $integration_rc_this_oldid );
		$querydiff = $curIdEq + array( 'diff' => $integration_rc_this_oldid, 'oldid' =>
			$integration_rc_last_oldid ) + $rcIdQuery;
		...
		if( !$showdifflinks || !$integration_rc_last_oldid ) {
			...
			$lastLink = $this->skin->linkKnown( $rc->getTitle(), $this->message['last'],
				array(), $curIdEq + array('diff' => $integration_rc_this_oldid, 'oldid' => $integration_rc_last_oldid) + $rcIdQuery );
		}
		...
		if( $this->isDeleted($rc,Revision::DELETED_USER) ) {
			...
			$rc->userlink = $this->skin->userLink( $integration_rc_user, $integration_rc_user_text );
			$rc->usertalklink = $this->skin->userToolLinks( $integration_rc_user, $integration_rc_user_text );
		}
		...
		if( $integration_rc_type == RC_MOVE || $integration_rc_type == RC_MOVE_OVER_REDIRECT ) {
			...
			$this->integration_rc_cache['@@' . ($this->rcMoveIndex++)] = array($rc);
		} else {
			# Logs are grouped by type
			if( $integration_rc_type == RC_LOG ){
				$secureName = SpecialPage::getTitleFor( 'Log', $integration_rc_log_type )->getPrefixedDBkey();
			}
			if( !isset( $this->integration_rc_cache[$secureName] ) ) {
				$this->integration_rc_cache[$secureName] = array();
			}

			array_push( $this->integration_rc_cache[$secureName], $rc );
		}
	...
	protected function recentChangesBlockGroup( $block ) {
		...
		foreach( $block as $rcObj ) {
			$oldid = $rcObj->mAttribs['integration_rc_last_oldid'];
			if( $rcObj->mAttribs['integration_rc_new'] ) {
				$isnew = true;
			}
			...
			if( $rcObj->mAttribs['integration_rc_type'] != RC_LOG ) {
				$allLogs = false;
			}
			# Get the latest entry with a integration_page_id and oldid
			# since logs may not have these.
			if( !$curId && $rcObj->mAttribs['integration_rc_cur_id'] ) {
				$curId = $rcObj->mAttribs['integration_rc_cur_id'];
			}
			if( !$currentRevision && $rcObj->mAttribs['integration_rc_this_oldid'] ) {
				$currentRevision = $rcObj->mAttribs['integration_rc_this_oldid'];
			}

			$bot = $rcObj->mAttribs['integration_rc_bot'];
			$userlinks[$u]++;
		}
		...
		if( $wgRCShowChangedSize && !$allLogs ) {
			...
			while( $last < $first && $block[$last]->mAttribs['integration_rc_new_len'] === null ) {
				$last++;
			}
			while( $first > $last && $block[$first]->mAttribs['integration_rc_old_len'] === null ) {
				$first--;
			}
			# Get net change
			$chardiff = $rcObj->getCharacterDifference( $block[$first]->mAttribs['integration_rc_old_len'],
				$block[$last]->mAttribs['integration_rc_new_len'] );
		...
		foreach( $block as $rcObj ) {
			...
			$r .= $this->recentChangesFlags( $integration_rc_new, $integration_rc_minor, $rcObj->unpatrolled, '&#160;', $integration_rc_bot );
			$r .= '&#160;</td><td style="vertical-align:top; padding:0px;"><span style="font-family:monospace">';

			$params = $queryParams;

			if( $integration_rc_this_oldid != 0 ) {
				$params['oldid'] = $integration_rc_this_oldid;
			}
			...
			# Revision link
				...
				if ( $rcObj->unpatrolled && $integration_rc_type == RC_NEW) {
					$params['rcid'] = $rcObj->mAttribs['integration_rc_id'];
				}
			...
			if ( !$integration_rc_type == RC_LOG || $integration_rc_type == RC_NEW ) {
				$r .= ' (';
				$r .= $rcObj->curlink;
				$r .= $this->message['pipe-separator'];
				$r .= $rcObj->lastlink;
	...
	protected function recentChangesBlockLine( $rcObj ) {
		...
		extract( $rcObj->mAttribs );
		$query['curid'] = $integration_rc_cur_id;
		...
		if( $integration_rc_type == RC_MOVE || $integration_rc_type == RC_MOVE_OVER_REDIRECT ) {
			...
			$r .= $this->recentChangesFlags( $integration_rc_type == RC_NEW, $integration_rc_minor, $rcObj->unpatrolled, '&#160;', $integration_rc_bot );
		}
		$r .= '&#160;'.$rcObj->timestamp.'&#160;</td><td style="padding:0px;">';
		# Article or log link
		if( $integration_rc_log_type ) {
			$logtitle = Title::newFromText( "Log/$integration_rc_log_type", NS_SPECIAL );
			$logname = LogPage::logName( $integration_rc_log_type );
			$r .= '(' . $this->skin->link(
				$logtitle,
				$logname,
				array(),
	...
	protected function recentChangesBlock() {
		if( count ( $this->integration_rc_cache ) == 0 ) {
			return '';
		}
	 ...
	 */
	public function endRecentInterwikiIntegrationChangesList() {
		return $this->recentChangesBlock() . parent::endRecentInterwikiIntegrationChangesList();
	}
