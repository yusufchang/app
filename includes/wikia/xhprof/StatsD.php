...
*
* @author etsy, Jean-François Bustarret, moli@wikia-inc.com
*/
...
class StatsD {
	...
	public function __construct( $host, $port, $proto = 'udp', $options = [] )
	{
		$this->host = $host;
		$this->port = $port;
		$this->proto = $proto;
		...
		if ( isset( $options[ 'packetSize' ] ) ) {
			$this->packetSize = $options[ 'packetSize' ];
		}
	}

	public function doNotTrack() { $this->doNotTrack = true; }
	...
	public function timing( $stat, $time, $sampleRate = 1 ) {
		$this->queue( [ $stat => "$time|ms" ], $sampleRate );
	}
	...
	protected function send( $data ) {
		...
		try {
			$fp = fsockopen( "{$this->proto}://{$this->host}", $this->port, $errno, $errstr );
			if ( !$fp ) { 
				return; 
			}
			fwrite( $fp, $data );
			...
			Wikia::log( __METHOD__, "", "cannot send data to StatsD");
		}
	}
	}
